...
model Order {
  ...
  clientName      String
  clientId        String?       // FK opcional para tabela de clientes
  status          OrderStatus   @default(NEGOTIATING)
  sellerId        String
  seller          User          @relation(fields: [sellerId], references: [id])
  
  // Valores agregados/calculados
  totalWeight     Decimal       @default(0) // kg
  averageMargin   Decimal       @default(0) // %
  totalValue      Decimal       @default(0) // R$
...
model OrderProduct {
  ...
  productId       String?  // FK opcional para catálogo
  name            String
  quantity        Int
  weight          Decimal  // kg unitário
  margin          Decimal  // %
  freight         Decimal  // R$
  unitPrice       Decimal  // R$
  totalPrice      Decimal  // R$ (quantity * unitPrice)
...
### 1.2. Comandos de Migration

```bash
# Criar migration
npx prisma migrate dev --name create_order_loss_tables
