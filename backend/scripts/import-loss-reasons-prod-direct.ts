...
async function createOrGetOrder(orderNumber: number, idUser: string, codRep: string): Promise<{ id: string | null, alreadyExists: boolean }> {
  try {
...
// Adiciona LossReason
async function addLossReason(orderId: string, code: 'FREIGHT' | 'PRICE' | 'MARGIN' | 'STOCK' | 'OTHER', description: string, submittedBy: string): Promise<boolean> {
  try {
