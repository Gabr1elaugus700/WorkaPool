...
**Código de embalagem**:
ERP packaging code on the Pedido item that identifies IBC packaging (e.g. **251004** = Container IBC). Used to decide which Pedidos appear on the driver’s unload screen. Distinct from Volume.
_Avoid_: Volume, treating packaging code as a quantity

**Volume (item do Pedido)**:
Numeric field on the Pedido item that is the **Quantidade esperada de IBC** for that line. Distinct from Código de embalagem.
_Avoid_: Código de embalagem, conflating Volume with liters/weight unless the ERP field is explicitly that quantity of containers
...
**Score do fator**:
Numeric score (0–10) given to one **Item de checklist** in an Inspeção de IBC (stored as IbcInspecaoResposta). Critical items (`critico`) have a hard minimum; others feed an overall average.
...
**Alocação na Carga**:
Planned link made at load assembly by the patio team: which IBCs are on a given Carga. Closing that launch moves those IBCs from company yard to **Em viagem**. Does not yet assert which Cliente holds them.
_Avoid_: Custódia no Cliente, treating Carga allocation as final customer location

**Em viagem**:
IBC custody state after Alocação na Carga is closed and before delivery confirmation or return entry. The asset is on the truck, not in the yard and not yet confirmed at a Cliente.
_Avoid_: No Cliente, Em estoque / no pátio

**Custódia no Cliente**:
Confirmed at unload per **Pedido**, not only per Cliente. Driver opens the Cliente stop, sees only Pedidos on that Carga that use IBC packaging (Código de embalagem), and for each Pedido scans/registers the IBCs that remain there, then saves (e.g. 3 IBCs for Pedido 1120, then 2 for Pedido 1121). Non-IBC Pedidos for the same Cliente are hidden. Records IBC + date + Cliente + Pedido.
_Avoid_: Alocação na Carga, Em viagem, dumping all of a Cliente’s IBCs without Pedido split when multiple IBC Pedidos exist
...
**Pedido com IBC**:
A Pedido (or Pedido line) whose Código de embalagem identifies Container IBC (e.g. **251004**). Only these appear on the driver’s unload screen for that Cliente.
_Avoid_: showing every Pedido of the Cliente on the IBC unload screen, using Volume to detect packaging type

**Quantidade esperada de IBC**:
How many IBCs that Pedido com IBC should involve on this trip — the item’s **Volume** field. Shown to the driver as the target (e.g. “esperado: 3”). Independent of Código de embalagem.
_Avoid_: inventing the count only in the driver’s head, reading quantity from the packaging code
...
_Avoid_: treating Alocação na Carga counts as Custódia no Cliente
