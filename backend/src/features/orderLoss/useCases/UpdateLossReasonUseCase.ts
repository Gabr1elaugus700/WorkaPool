...
export class UpdateLossReasonUseCase {
  ...
  async execute(data: UpdateLossReasonDTO): Promise<LossReason> {
    ...
    if (order.status !== OrderStatus.LOST) {
      throw new AppError({
        message: "Pedido não está em estado LOST",
        statusCode: 409,
        code: "ORDER_NOT_LOST",
      });
    }
    ...
    if (ageMs > msIn7Days) {
      throw new AppError({
        message: "Passou do prazo de 7 dias para atualizar a justificativa",
        statusCode: 403,
        code: "LOSS_REASON_EXPIRED",
      });
    }
