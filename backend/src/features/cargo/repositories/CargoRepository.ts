...
export class CargoRepository implements ICargoRepository {
    ...
    const result = await sqlPool.request().input("codCar", codCar)
      .query<PedidoRaw>(`
                ...
                GROUP BY ped.numped, ped.codcli, cli.nomcli, cli.cidcli, cli.sigufs,
                         ...
                         grp.desgrp, ped.usu_codcar, ped.usu_poscar, ped.usu_sitcar
            `);
  ...
  async getPedidosFechadosPorVendedor(codRep: number): Promise<Pedido[]> {
    await sqlPoolConnect;

    const isAll = codRep === 999 || codRep === null || codRep === undefined;
  ...
  async updateSituacaoCarga(codCar: number, situacao: SituacaoCarga): Promise<Carga> {
