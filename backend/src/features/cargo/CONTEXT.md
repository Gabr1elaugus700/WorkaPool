...
**CargaDespacho**:
Dispatch record created when a Carga is closed: links cargo to one truck. Stored in an intermediate table (not inline on `Cargas`) for future extensibility. 1:1 per cargo. No motoristaId.
_Avoid_: Caminhao (deprecated), multiple trucks per cargo in v1, motorista obrigatório no close
